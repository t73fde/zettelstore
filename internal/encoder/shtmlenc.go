...
func (enc *shtmlEncoder) WriteZettel(w io.Writer, zn *zettel.ParsedZettel) error {
	env := shtml.MakeEnvironment(enc.lang)
	metaSHTML, err := enc.th.EvaluateMetadata(zn.InhMeta, &env)
	if err != nil {
		return err
	}
...
func (enc *shtmlEncoder) WriteMeta(w io.Writer, m *meta.Meta) error {
	env := shtml.MakeEnvironment(enc.lang)
	metaSHTML, err := enc.th.EvaluateMetadata(m, &env)
	if err != nil {
		return err
	}
